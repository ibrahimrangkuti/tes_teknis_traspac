function hitungKata(artikel, kata) {
    const words = artikel.split(/\s+/)
    let count = 0

    for (let k of words) {
        if (k.toLowerCase() === kata.toLowerCase()) count++
    }

    return count
}

const artikel = `Dalam kehidupan suatu negara, pendidikan memegang peranan yang amat 
...
const cariKata = "negara"
const jumlahKata = hitungKata(artikel, cariKata)

console.log(`Pencarian kata "${cariKata}" ada sebanyak ${jumlahKata} kali dalam artikel.`)